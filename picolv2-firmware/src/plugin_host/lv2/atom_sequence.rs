...
pub(in crate::plugin_host) const MIDI_BLOCK_CAPACITY: usize = 64;
const ATOM_SEQUENCE_CAPACITY: usize = 2048;

#[repr(C)]
#[derive(Clone, Copy)]
pub(in crate::plugin_host) struct Lv2Atom {
    pub size: u32,
    pub atom_type: u32,
}
...
#[repr(C)]
#[derive(Clone, Copy)]
pub(in crate::plugin_host) struct Lv2AtomSequenceBody {
    pub unit: u32,
    pub pad: u32,
}

#[repr(C)]
pub(in crate::plugin_host) struct Lv2AtomSequence {
    pub atom: Lv2Atom,
...
impl Lv2AtomSequence {
    pub(in crate::plugin_host) const fn empty() -> Self {
        Self {
            atom: Lv2Atom {
    ...
    pub(in crate::plugin_host) fn clear(&mut self) {
    ...
    pub(in crate::plugin_host) fn set_capacity(&mut self) {
        self.atom.size =
    ...
    pub(in crate::plugin_host) fn push_midi(&mut self, frame: i64, message: [u8; 3]) -> bool {
        self.push_event(frame, MIDI_EVENT_URID, &message)
    }

    pub(in crate::plugin_host) fn push_position(
        &mut self,
        absolute_frame: u64,
        sample_rate: u32,
        bpm: f32,
